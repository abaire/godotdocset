...
def j_print_args(args, **_kwargs):
    ...
    return ", ".join(
        [
            f'{get_type(a.type)} {a.name}{"="+a.default if a.default else ""}'
            for a in args
        ]
    )
...
class DocPage:
    ...
    def build_parents(self):
        ...
        if name not in DocPage.parents:
            print(
                "Failed to find parent %s for %s" % (name, self.title), file=sys.stderr
            )
            return
        while DocPage.parents[name]:
            ...
            if name not in DocPage.parents:
                print(
                    "Failed to find parent %s for %s" % (name, self.title),
                    file=sys.stderr,
                )
                break
    ...
    def populate_methods(self):
        ...
        for method in self._root.findall("methods/method"):
            ...
            res.__dict__["return"] = (
                method.find("return").attrib["type"]
                if method.find("return") is not None
                else "void"
            )
    ...
    def populate_consts(self):
        ...
        for const in self._root.findall("constants/constant"):
            ...
            if ename not in self.consts:
                self.consts[ename] = []
            res = SimpleNamespace(
                name=const.attrib["name"], value=const.attrib["value"]
            )
...
""".format(
        name.split("_")[0], name.replace("_", " "), name.split("_")[0].lower()
    )
...
def main():
    ...
    with DocsetMaker() as docset:
        ...
        def dump(content, name):
            with open(
                os.path.join(DocsetMaker.docdir, name[:-4] + ".html"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(content)
